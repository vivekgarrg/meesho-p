"""
Grant /daily-profit to already-configured access rules.

Same reasoning as 0005, 0006 and 0007: a rule is an explicit allow-list, so one
written before the Daily Profit tab existed hides it — the tab would otherwise
be invisible to anyone with a restrictive rule already saved, even though the
underlying profit/ endpoints are unguarded (see accounts/nav.py API_OWNERSHIP).
"""

from django.db import migrations

NEW_PATHS = ["/daily-profit"]
