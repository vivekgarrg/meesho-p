...
class BulkListingBatch(models.Model):
    """
    One generated bulk-listing sheet, kept so it can be re-downloaded byte-
    for-byte and reloaded into the form for editing without redoing the data
    entry. `file_data` holds the literal generated bytes (not regenerated on
    demand) because the source template can change or be deleted later, and
    two separate openpyxl writes aren't guaranteed byte-identical anyway —
    only the original response body truly is "the same file as it is".

    Also seeds one WorkerTask + one TaskListing per SKU (see
    bulk_listing_views.bulk_listing_generate) so every generated SKU is
    reviewed and paid through the existing Team Tasks pipeline. This row is
    the paper trail connecting "a sheet got generated" to "N SKUs are now
    pending admin approval".
    """

    PLATFORM_MEESHO = "meesho"
    PLATFORM_FLIPKART = "flipkart"
    PLATFORM_CHOICES = [(PLATFORM_MEESHO, "Meesho"), (PLATFORM_FLIPKART, "Flipkart")]
    ...
    SOURCE_CHOICES = [
        ...
        (SOURCE_TEMPLATE, "Saved template"),
    ]
    ...
    # ── what to hand back on re-download ──────────────────────────────────
    filename  = models.CharField(max_length=255)
    file_data = models.BinaryField()

    # ── what to hand back on "load to edit" ───────────────────────────────
    source_kind = models.CharField(max_length=20, choices=SOURCE_CHOICES)
    source_built_in_key = models.CharField(max_length=100, blank=True)
    source_template = models.ForeignKey(
        "FlipkartBulkTemplate", on_delete=models.SET_NULL, null=True, blank=True,
        related_name="bulk_listing_batches",
    )
    source_file_data = models.BinaryField(null=True, blank=True)
    source_original_filename = models.CharField(max_length=255, blank=True)
    # {mode, shared, rows, image_urls} — exactly the payload the generate
    # endpoint already receives, so no new shape to invent for "load to edit".
    payload_snapshot = models.JSONField(default=dict, blank=True)

    # ── denormalised for the list table, so rendering a row never needs to
    # unpack file_data ──────────────────────────────────────────────────────
    first_sku_id = models.CharField(max_length=300, db_index=True)
    sku_ids = models.JSONField(default=list, blank=True)
    row_count = models.PositiveIntegerField(default=0)

    # Denormalised link to the auto-created WorkerTask, so the panel can show
    # review status ("3/12 approved") without joining through task_listings.
    worker_task = models.ForeignKey(
...
class TaskListing(models.Model):
    ...
    # Set when this SKU came from a Bulk Listing generation rather than the
    # manual "add one SKU to a task" form. SET_NULL rather than CASCADE/
    # PROTECT — a paid, approved listing must never be destabilised if the
    # batch record it came from is ever removed.
    batch = models.ForeignKey(
        "BulkListingBatch", on_delete=models.SET_NULL, null=True, blank=True,
        related_name="task_listings",
    )
