# Meesho & Flipkart Dynamic Lister — Chrome Extension

A Manifest V3 Chrome extension that **auto-detects the fields on any Meesho or Flipkart Seller Hub listing/catalog form**, lets you **save those values as named, reusable templates on your Rudam account**, **prefills an entire form in one click**, and **generates product images** from a text prompt.

Built to be resilient to UI changes on either site: nothing is hardcoded to specific Meesho or Flipkart element IDs — see "Why a logical key" below.
...
**Prefill a listing**
1. Open a Meesho add-product / catalog form, or a Flipkart Seller Hub (`seller.flipkart.com`) listing form.
2. Click the extension → **Scan page**. Detected fields appear with their labels.
3. Edit values inline if needed → **Prefill all**.

A template only ever fills fields whose logical key matches the page it's applied to, so a Meesho template harmlessly skips everything on a Flipkart page (and vice versa) rather than filling the wrong thing. The Templates tab tags each one with the site it was captured from so you can tell at a glance.
...
content/
  fieldScanner.js          Pure detection/read/write engine (window.MeeshoFieldScanner), shared by both sites
  content.js               Message bridge popup <-> page, floating badge (labelled per-site), highlight
popup/
...
**Why a logical key, not a CSS selector.** Meesho and Flipkart Seller Hub are both React apps with minified, deploy-varying class names. Templates are keyed on a *stable logical key* derived from `name` → `id` → label → placeholder, so a saved template keeps matching fields after either site ships a redesign — and so the same scanner works on both sites without knowing which one it's on. See `deriveKey()` in `fieldScanner.js`.
...
- Host access is limited to `meesho.com`, `seller.flipkart.com`, your Rudam server, and the two image-API domains.
...
- Tested against Manifest V3 on Chromium. Selectors are heuristic, not selectors against Meesho or Flipkart internals, so they should survive most UI updates.
- Flipkart Seller Hub support reuses the same generic detection as Meesho (labels, `aria-*`, `role`, MUI-style dropdown hints, and a handful of generic dropdown/menu class-name hints). If a particular Flipkart dropdown widget doesn't fill correctly, use **Locate** to find it and report which field it is — the dropdown-matching heuristics in `fieldScanner.js` may need a Flipkart-specific hint added.
